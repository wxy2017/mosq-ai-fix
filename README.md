...
```
  ...
  > `dist\.cache\` 为运行时自动生成的磁盘缓存（存历史润色文本，用于二次调用加速），
  > **不是交付物**：打包发给别人前建议删掉整个 `.cache` 目录。
...
### 【验证部署是否正常】
  在 dist 目录下用命令行跑一次自检（会读配置并做一次真实润色，结果打印到控制台）：
```
cd dist\runtime
...
```
  ...
  若显示 `__NO_KEY__` / `__ERROR__` 前缀 → 表示密钥未配置或网络不可达。
